...
<div class="paises index list row card-panel">
    ...
    <?= $this->Gus->create('Pais', [
        'url' => ['controller' => 'paises', 'action' => 'delete'],
        'method' => 'post',
        'class' => 'hide',
        'id' => 'form-delete',
    ]); ?>
    ...
    <table class="responsive-table index highlight">
        ...
        <?php foreach ($paises as $pais): ?>
            <tr>
                ...
                <td><input type="checkbox" id="check<?= $pais->id ?>" class="filled-in" name="data[ids][<?= $pais->id ?>]" value="<?= $pais->id ?>" /><label for="check<?= $pais->id ?>">&nbsp;</label></td>
                <td><?= h($pais->nome) ?></td>
                <td><?= h($pais->sigla) ?></td>
                <td><?= $this->Gus->formataStatus($pais->status) ?></td>
                <td class="actions">
                    <?= $this->Html->link($this->Gus->materialIcon('edit'), ['action' => 'edit', $pais->id], ['escape' => false, 'class' => 'btn btn-floating btn-sm waves-effect waves-light edit']) ?>
                </td>
            </tr>
        <?php endforeach; ?>
        </tbody>
...
<script>
    var addLink = '<?= Router::url(['controller' => 'paises', 'action' => 'add'], true); ?>';
    var editLink = '<?= Router::url(['controller' => 'paises', 'action' => 'edit', '--id--'], true); ?>';
    var viewLink = '<?= Router::url(['controller' => 'paises', 'action' => 'view', '--id--'], true); ?>';
    var deleteLink = '<?= Router::url(['controller' => 'paises', 'action' => 'delete'], true); ?>';
</script>
